...
def getInstallerInfo(target):
    ...
    if 'install.cfg' not in z.namelist() or \
        all(name.startswith('app/mod/') or name.startswith('res/') \
...
def exportModule(mod, target, export_source=False):
    ...
    if export_source:
        os.path.walk('app/mod/'+mod.name, lambda *args: zipdirectory(*args, ignore_hidden=True), z)
    else:
        z.writepy('app/mod/'+mod.name, 'app/mod')
